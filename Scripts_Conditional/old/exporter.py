...
# Get the default value of a node socket
...
# Save to dir if exists
output_directory = bpy.path.abspath("//")  # set path
if not os.path.exists(output_directory):
    os.makedirs(output_directory)

# Iterate over all objects in the scene???
for obj in scene.objects:
    print("\nObject: ", obj.name)  # Print name DEBUG
    if obj.type == 'MESH':
            ...
            for mat in obj.data.materials:
                if mat is not None:
                    ...
                    try:
                        ...
                        print(f"Error writing data to file {file_name}: {e}")
                    print(material_info)
        else:
